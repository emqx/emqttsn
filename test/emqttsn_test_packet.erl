  ...
  [{parse, [parallel],
   ...
   {connect, [parallel],
    [t_serialize_parse_connect,
     t_serialize_parse_connect_without_will
    ]},
   {connack, [parallel],
...
t_parse_short_length(_) ->
  ...
  {Length, Rest} = emqttsn_frame:parse_leading_len(Packet, #config{}),
...
t_parse_frame_too_large(_) ->
  Packet = ?PUBLISH_PACKET(?TOPIC_ID, 1, payload(1000)),
  ?catch_error(frame_too_large, emqttsn_frame:parse(Packet, #config{max_size = 256})),
  ?catch_error(frame_too_large, emqttsn_frame:parse(Packet, #config{max_size = 512})),
  ?assertEqual(Packet, emqttsn_frame:parse(Packet, #config{max_size = 1024})).
...
t_serialize_parse_connect(_) ->
  ...
  {ok, Packet} = emqttsn_frame:parse(Bin, #config{}),
